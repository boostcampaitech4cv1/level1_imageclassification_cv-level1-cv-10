    ...
    def __getitem__(self, idx):
        ...
        if gen == 0 & age_category == 0:
            ...
            for m in range(len_transform):
                ...
                #image0.save(''.join([file_name,f'_aug{i}.jpg']))  
        print(gen,age_category)
        return 'New Valance!'

if __name__ == "__main__":
    train_dir = "/opt/ml/input/data/train"

    male_30_transform = torch.nn.Sequential(         # 3 augmentations
    ...
    female_30_transform = torch.nn.Sequential(       # 2 augmentations
        
            transforms.RandomHorizontalFlip(p=1.0),
            transforms.ColorJitter(brightness= .5,contrast=0.5,saturation=0.5,hue=0.3)
        
    )
    male_3060_transform = torch.nn.Sequential(      # 4 augmentations
    ...
    female_3060_transform = torch.nn.Sequential(    # 2 augmentations
        
            transforms.RandomHorizontalFlip(p=1.0),
            transforms.ColorJitter(brightness= .5,contrast=0.5,saturation=0.5,hue=0.3)      
        
    )
    male_60_transform = torch.nn.Sequential(        # 10 augmentaions
    ...
    female_60_transform = torch.nn.Sequential(      # 10 augmentaions
